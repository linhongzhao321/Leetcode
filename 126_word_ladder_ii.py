...
class Solution(object):
    ...
    def findLadders(self, begin_word, end_word, word_list: list):
        """
        :type begin_word: str
        :type end_word: str
        ...
        :rtype: List[List[str]]
        """
        not_visited = Counter(word_list)
        if not_visited[begin_word] == 1:
            word_list.remove(begin_word)
        if not_visited[end_word] == 0:
            return []
        word_len = len(begin_word)
        if word_len == 1:
            return [[begin_word, end_word]]

        not_visited[begin_word] = 0
                ...
                while k < word_len:
                    if str1[k] != str2[k]:
                        ...
                        if dis > 1:
                            break
                    k += 1
                if ham_near_map.get(str1) is None:
                    ham_near_map[str1] = {str1: False}
                if ham_near_map.get(str2) is None:
                    ham_near_map[str2] = {str2: False}
                ham_near_map[str1][str2] = k == word_len
                ham_near_map[str2][str1] = k == word_len
                j += 1
            i += 1

        if ham_near_map[begin_word][end_word]:
            return [[begin_word, end_word]]
        # BFS
        queue = deque([begin_word])
        results = []
        for (key, is_near) in ham_near_map[begin_word].items():
            if is_near:
                results.append([begin_word, key])
        is_finish = False
        while len(queue) > 0:
            print(len(queue))
            cur = queue.popleft()
            if cur == begin_word:
                ...
                # print(results)
            for (key, is_near) in ham_near_map[cur].items():
                if ham_near_map[cur][end_word]:
                    is_finish = True
                    break
                if is_near and not_visited[key] >= 1:
                    not_visited[key] -= 1
                    queue.append(key)
            # if is_finish:
            #     break
        ...
        min_cnt = 999999
        for result in results:
            if ham_near_map[result[-1]][end_word]:
...
if __name__ == '__main__':
    ...
    # print(s.findLadders("teach",
    #                     "place",
    #                     ["peale", "wilts", "place", "fetch", "purer", "pooch", "peace", "poach", "berra", "teach",
    #                      "rheum", "peach"]))
